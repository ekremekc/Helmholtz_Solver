...
class ActiveFlame:
    ...
    def _assemble_left_vector(self, fl):
        ...
        # h_e = dolf.Expression(string_f_1d, degree=0, x_0=self.x_r[0][0], sigma=0.025)
        # h_u = dolf.interpolate(h_e,V)
        # h_array = np.repeat(h_u.vector()[:],2)
        # h_matrix = np.zeros((len(h_array), len(h_array)))
        # np.fill_diagonal(h_matrix, h_array)

        # w_e = dolf.Expression(string_f_1d, degree=0, x_0=self.x_r[0][0], sigma=0.025)
        # w_u = dolf.interpolate(w_e,V)
        # w = dolf.assemble(w_u*dx1)
        # rho = dolf.Expression("rho_u+0.5*(rho_d-rho_u)*(1+tanh((x[0]-x_f)/a_f))", degree=1,
        #          rho_u = 1.4,
        #          rho_d = 0.975,
        #          x_f = self.x_f[0][0],
        #          a_f = 0.025)
        # string_w_1d = '1 / (sigma * sqrt(2*pi)) * exp(- pow(x[0] - x_0, 2) / (2 * pow(sigma, 2)) )'
        # w_r = dolf.Expression(string_w_1d, degree=0, x_0=self.x_r[0][0], sigma=0.025)
        # w_r = dolf.interpolate(w_r,V)


        # newArray = np.zeros(2*len(w_r.vector()[:]),dtype=int)
        # newArray[::2] = w_r.vector()[:]
        # ww = dolf.Function(V)
        # ww.vector()[:]=newArray[:]
        ...
        # a_1 = np.matmul( dolf.assemble(v_1 / V_fl * dx(fl)), mult)  
        # a_2 = np.matmul( dolf.assemble(v_2 / V_fl * dx(fl)), mult)
