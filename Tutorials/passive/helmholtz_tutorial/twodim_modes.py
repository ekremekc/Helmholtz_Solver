...
def mshr_(L, h, nx, ny):
    ...
    class Bottom(dolf.SubDomain):
        def inside(self, x, on_boundary):
            return on_boundary and dolf.near(x[1], 0)

    class Top(dolf.SubDomain):
        def inside(self, x, on_boundary):
            return on_boundary and dolf.near(x[1], h)
    ...
    # domain = Rectangle(dolf.Point(0,0), dolf.Point(L,h))
...
def onedim_eig(Z, c_0, L, target, index=1):
    ...
    mesh, boundaries = mshr_(L, h, nx, ny)
...
# print(onedim_eig(-2, c_0, L,k0, index=1))
...
# plt.plot(x_num, numeric_a_imag)
# plt.plot(x_num, numeric_b_real)
# plt.plot(x_num, numeric_a_real)
