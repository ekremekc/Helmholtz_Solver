...
# rho = dolf.Expression('x[0] <= x_f ? rho_in : rho_out', degree=0, x_f=x_f[0][0], rho_in=rho_in, rho_out=rho_out)
...
c_p = dolf.Expression(string_f_1d, degree=0, x_0=x_r[0][0], sigma=a_f)

c_perturbed = dolf.Expression("sqrt(gamma*p_amb/rho)-0.000*perturbation", degree = 1,
               gamma = gamma,
               p_amb = p_amb/p_ref,
               rho = rho,
...
c_plot = dolf.interpolate(c_perturbed, V)
dolf.plot(c_plot)
plt.show()
print("line worked")