...
class Class1:
    ...
    def __init__(self, geometry, boundary_conditions, p_dir, p_adj, c):
        """
        Axisymmetric Shape derivative class for geometry which has Robin BC
        ...
        self.k = Expression('1/(0.00000001+x[1])', degree=1)
    ...
    def integral_3(self):
        """
        The difference between planar case and axisymmetric case occurs here. 
        Because divergence term differs for axisymmetric case.
        part[0] refers z component of the cylindrical domain
        part[1] refers \rho component of the cylindrical domain
        
        \div = (1/\rho)*\partial(\rho P[1])/\partial(\rho) + \partial(P[0])/\partial(\rho)
            
                    equals to
                    
        \div = \partial(P[1])/\partial(\rho) + P[1]/(\rho) + \partial(P[0])/\partial(\rho)
        ...
        real_part = curvature * dot(real_part, n) + \
                    real_part[0].dx(0) + k * real_part[1] + real_part[1].dx(1) - \
                    dot(dot(grad(real_part), n), n)
        imag_part = curvature * dot(imag_part, n) + \
                    imag_part[0].dx(0) + k * imag_part[1] + imag_part[1].dx(1) - \
                    dot(dot(grad(imag_part), n), n)
    ...
    def __call__(self, i):
        ...
        #     for V_ in V:
        #         for expr_ in expr:
        #             mylist.append(assemble(dot(V_, n) * expr_ *self.r* ds(i)))
            # return mylist
        ...
        for j in range(len(ctrl_pts)):
            ...
            if "Dirichlet" in self.boundary_conditions[i]:
                ...
                real_x = assemble( dot(V_x, n) * Dir_real *self.r* ds(i) )
                imag_x = assemble( dot(V_x, n) * Dir_imag *self.r* ds(i) )

                real_y = assemble( dot(V_y, n) * Dir_real *self.r* ds(i) )
                imag_y = assemble( dot(V_y, n) * Dir_imag *self.r* ds(i) )

            elif "Neumann" in self.boundary_conditions[i]:

                (Neu_real, Neu_imag) = self.get_Neumann()

                real_x = assemble( dot(V_x, n) * Neu_real *self.r* ds(i) )
                imag_x = assemble( dot(V_x, n) * Neu_imag *self.r* ds(i) )

                real_y = assemble( dot(V_y, n) * Neu_real *self.r* ds(i) )
                imag_y = assemble( dot(V_y, n) * Neu_imag *self.r* ds(i) )

            elif "Robin" in self.boundary_conditions[i]:

                (Rob_real, Rob_imag) = self.get_Robin()

                real_x = assemble( dot(V_x, n) * Rob_real *self.r* ds(i) )
                imag_x = assemble( dot(V_x, n) * Rob_imag *self.r* ds(i) )

                real_y = assemble( dot(V_y, n) * Rob_real *self.r* ds(i) )
                imag_y = assemble( dot(V_y, n) * Rob_imag *self.r* ds(i) )
